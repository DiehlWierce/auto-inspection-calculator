...
import { money, numberValue } from '../utils';
import { ACCIDENT_OUTCOME_LABELS, DOCUMENT_LABELS } from '../labels';
import { INSPECTION_STAGES, applicableTemplates, makeInspection, modelName, modelProfile } from '../domain/inspection';
import { Field, FormSection } from '../components/primitives';
import type { AppConfig, Inspection, ModelId, VehicleInfo } from '../types';

export function NewInspectionView({
  config,
  onCancel,
  onCreate,
}: {
  config: AppConfig;
  onCancel: () => void;
  onCreate: (inspection: Inspection) => void;
}) {
  ...
  const selectedVariant =
    selectedModel.engineVariants.find((variant) => variant.id === engineVariantId) ?? selectedModel.engineVariants[0];
  ...
  const submit = (event: FormEvent) => {
    ...
    onCreate(
      makeInspection(config, {
        ...
        actualPurchasePrice: actual ? numberValue(actual) : undefined,
        vin,
        plate,
        listingUrl: url,
        listingSource: source,
        documentsStatus,
        keyCount: keyCount === '' ? undefined : numberValue(keyCount),
        accidentStatus,
        accidentOutcomes,
        accidentComment,
        templateId: selectedTemplate?.id,
        inspectionLayout: selectedTemplate?.layout ?? INSPECTION_STAGES,
      }),
    );
  };
  ...
  return (
    <section className="page-section narrow-page">
      <div className="page-heading">
        <div>
          <p className="eyebrow">ШАГ 1 / АВТОМОБИЛЬ</p>
          <h1>Новый осмотр</h1>
          <p className="muted">Выберите автомобиль из фиксированного пула и укажите цену сценария.</p>
        </div>
      ...
      <form className="form-card" onSubmit={submit}>
          ...
          <div className="model-picker">
            {config.models.map((model) => (
              <button
                type="button"
                key={model.id}
                className={`model-option ${model.id === modelId ? 'selected' : ''}`}
                onClick={() => changeModel(model.id)}
              >
                <span className="model-radio"></span>
                <span>
                  <strong>{modelName(config, model.id)}</strong>
                  <small>
                    {model.engine} · {model.transmission} · демо-расход {model.consumptionLPer100Km} л/100 км
                  </small>
                </span>
              </button>
            ))}
          ...
          <div className="form-grid four">
            <Field label="Год">
              <input
                ...
                max="2030"
                value={year}
                onChange={(event) => setYear(numberValue(event.target.value))}
              />
            </Field>
            <Field label="Пробег, км">
              <input
                ...
                min="0"
                value={mileage}
                onChange={(event) => setMileage(numberValue(event.target.value))}
              />
            </Field>
            <Field label="Источник">
              <input value={source} onChange={(event) => setSource(event.target.value)} placeholder="Avito" />
            </Field>
            <Field label="Двигатель и ГРМ">
              <select value={engineVariantId} onChange={(event) => setEngineVariantId(event.target.value)}>
        ...
        <FormSection title="Шаблон осмотра">
          {availableTemplates.length > 0 ? (
            <>
              <Field label="Набор этапов и элементов">
              ...
              <p className="field-note">{selectedTemplate?.description} Можно изменить структуру уже внутри осмотра.</p>
            </>
          ) : (
            <p className="field-note">
              Для этой модели пока нет сохранённого шаблона. Будет использован классический набор этапов, который можно
              настроить вручную.
            </p>
          )}
        </FormSection>
        <FormSection title="Цена и объявление">
          <div className="form-grid three">
            <Field label="Цена объявления, ₽" hint={`Фильтр: ${money(config.maxAskingPrice)}`}>
              <input
                type="number"
                min="0"
                value={asking}
                onChange={(event) => setAsking(numberValue(event.target.value))}
                required
              />
            </Field>
            ...
            <Field label="Фактическая цена, ₽" hint="Необязательно">
              <input
                type="number"
                min="0"
                value={actual}
          ...
          <div className="form-grid three">
            <Field label="Документы">
              <select
                ...
                onChange={(event) =>
                  setDocumentsStatus(event.target.value as NonNullable<VehicleInfo['documentsStatus']>)
                }
              >
            ...
            <Field label="История ДТП">
              <select
                value={accidentStatus}
                onChange={(event) =>
                  setAccidentStatus(event.target.value as NonNullable<VehicleInfo['accidentStatus']>)
                }
              >
          ...
          {accidentStatus === 'YES' && (
            <>
                ...
                <div className="check-grid">
                  {Object.entries(ACCIDENT_OUTCOME_LABELS).map(([outcome, label]) => (
                    <label key={outcome} className="check-item">
                      <input
                        type="checkbox"
                        checked={accidentOutcomes.includes(outcome)}
              ...
              <Field label="Комментарий по ДТП">
                <textarea
                  rows={2}
                  value={accidentComment}
                  onChange={(event) => setAccidentComment(event.target.value)}
                />
              </Field>
            </>
          )}
