...
export function Summary({ result, inspection, onForecast }: { result: CalculationResult; inspection: Inspection; onForecast: () => void }) {
  ...
  return <div className={`summary-card ${result.zone.toLowerCase()}`}>
    ...
    <div className={`information-note ${result.questionFactsCount > 0 ? 'question-note' : ''}`}><strong>Определённость осмотра</strong><span>{informationText}</span>{result.estimatedFactsCount > 0 && <span>Работ со стоимостью по справочнику: {result.estimatedFactsCount}. В смете — типовая цена вилки, в безопасной — максимум. Это не штраф, но расчёт остаётся предварительным.</span>}{result.unknownCostCount > 0 && <span>Работ без цены и без вилки: {result.unknownCostCount}. Такие факты блокируют расчёт.</span>}</div>
