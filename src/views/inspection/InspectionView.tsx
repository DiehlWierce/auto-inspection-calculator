...
export function InspectionView({ inspection, result, onUpdate, onNavigate }: { inspection: Inspection; result: CalculationResult; onUpdate: (inspection: Inspection) => void; onNavigate: (view: View) => void }) {
  ...
  return <section className="page-section inspection-page">
    ...
    {activeStage && <StageReview key={activeStage.id} stage={activeStage} stepIndex={stages.findIndex((item) => item.id === activeStage.id)} facts={result.calculatedFacts} config={inspection.configSnapshot} onSaveFact={upsertFact} onDeleteFact={deleteFact} onOpenPriceBook={() => onNavigate('settings')} />}
    ...
    {showFreeForm && <FactForm form={form} editing={Boolean(editingId)} config={inspection.configSnapshot} onChange={setForm} onCategoryChange={changeCategory} onToggleRisk={toggleRisk} onCancel={resetForm} onSubmit={saveFact} onOpenPriceBook={() => onNavigate('settings')} />}
