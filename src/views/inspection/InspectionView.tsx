...
import { engineVariant, modelName } from '../../domain/vehicle';
import { CriticalPoint } from './CriticalPoint';
import { FactCard, FactGroupSummary } from './FactCard';
import { FactForm } from './FactForm';
import { InspectionLayoutEditor } from './InspectionLayoutEditor';
import { StageReview } from './StageReview';
import { StageTab } from './StageTabs';
import { Summary } from './Summary';
import { VehicleEditor } from './VehicleEditor';
import { VehicleInfoSummary } from './VehicleInfoSummary';
import type { BodyRisk, CalculationResult, CategoryId, Fact, Inspection, View } from '../../types';

export function InspectionView({ inspection, result, onUpdate, onNavigate }: { inspection: Inspection; result: CalculationResult; onUpdate: (inspection: Inspection) => void; onNavigate: (view: View) => void }) {
  ...
  return <section className="page-section inspection-page">
    ...
    {activeStage && <StageReview key={activeStage.id} stage={activeStage} stepIndex={stages.findIndex((item) => item.id === activeStage.id)} facts={result.calculatedFacts} config={inspection.configSnapshot} modelId={inspection.vehicle.modelId} onSaveFact={upsertFact} onDeleteFact={deleteFact} onOpenPriceBook={() => onNavigate('settings')} />}
    ...
    {showFreeForm && <FactForm form={form} editing={Boolean(editingId)} config={inspection.configSnapshot} modelId={inspection.vehicle.modelId} onChange={setForm} onCategoryChange={changeCategory} onToggleRisk={toggleRisk} onCancel={resetForm} onSubmit={saveFact} onOpenPriceBook={() => onNavigate('settings')} />}
