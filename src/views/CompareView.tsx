...
export function CompareView({ inspections, results, config, onOpen }: { inspections: Inspection[]; results: Map<string, CalculationResult>; config: AppConfig; onOpen: (id: string) => void }) {
  const [sort, setSort] = useState('score');
  const rows = useMemo(() => inspections.flatMap((inspection) => { const result = results.get(inspection.id); return result ? [{ inspection, result }] : []; }).sort((left, right) => {
