...
function LegacyStageReview({ stage, stepIndex, facts, onSaveFact, onDeleteFact }: { stage: InspectionStage; stepIndex: number; facts: ReturnType<typeof calculateInspection>['calculatedFacts']; onSaveFact: (fact: Fact) => void; onDeleteFact: (id: string) => void }) {
...
function LegacyForecastBreakdown({ forecast }: { forecast: ReturnType<typeof calculateInspection>['forecast'] }) {
  ...
  return <div className="forecast-breakdown">
    <div className="table-scroll"><table className="year-summary-table"><thead><tr><th>Год</th><th>Регулярные расходы</th><th>В резерв</th><th>Отложенные работы</th><th>События</th><th>Ожидаемо всего</th><th>Крупный ремонт</th><th>Риск лимита</th></tr></thead>{forecast.years.map((year) => { const months = forecast.months.filter((month) => month.year === year.year); const regular = months.reduce((sum, month) => sum + month.regularExpenses, 0); const reserve = months.reduce((sum, month) => sum + month.plannedReserve, 0); const scheduled = months.reduce((sum, month) => sum + month.scheduledEvents + month.expectedRepairs, 0); return <tbody key={year.year}><tr><td><button className="year-toggle" onClick={() => toggleYear(year.year)}>{expandedYears[year.year] ? '−' : '+'} Год {year.year}</button></td><td>{money(regular)}</td><td>{money(reserve)}</td><td>{money(year.deferredFacts)}</td><td>{money(scheduled)}</td><td><strong>{money(year.expectedTotal)}</strong></td><td><span className={year.probabilityAnyMajorRepair > 0 ? 'table-warn' : 'table-good'}>{percent(year.probabilityAnyMajorRepair)}</span></td><td><span className={year.probabilityLimitViolation > 0 ? 'table-warn' : 'table-good'}>{percent(year.probabilityLimitViolation)}</span></td></tr>{expandedYears[year.year] && <tr><td colSpan={8}><MonthlyForecastTable months={months} /></td></tr>}</tbody>; })}</table></div>
    <p className="forecast-footnote">Баланс резерва — расчётная сумма: пополнение по плану минус ожидаемые разовые траты. Это помогает увидеть, в каком месяце накопленного резерва может не хватить; это не банковский счёт и не гарантия фактического ремонта.</p>
  </div>;
}

function LegacyMonthlyForecastTable({ months }: { months: ReturnType<typeof calculateInspection>['forecast']['months'] }) {
  return <div className="monthly-table-wrap"><table className="monthly-table"><thead><tr><th>Месяц</th><th>Бензин</th><th>ОСАГО / налог</th><th>ТО / жидкости</th><th>Расходники / резина</th><th>Мойка / штрафы</th><th>Регулярно</th><th>В резерв</th><th>Отложенные</th><th>По сроку</th><th>Риски, ожидаемо</th><th>Всего</th><th>Баланс резерва</th></tr></thead><tbody>{months.map((month) => <tr key={month.month}><td><strong>{month.month}</strong></td><td>{money(month.fuel)}</td><td>{money(month.insurance + month.tax)}</td><td>{money(month.service + month.fluids)}</td><td>{money(month.consumables + month.tires)}</td><td>{money(month.washing + month.fines)}</td><td>{money(month.regularExpenses)}</td><td className="reserve-cell">{money(month.plannedReserve)}</td><td>{month.deferredFacts > 0 ? money(month.deferredFacts) : '—'}</td><td>{month.scheduledEvents > 0 ? money(month.scheduledEvents) : '—'}</td><td>{month.expectedRepairs > 0 ? money(month.expectedRepairs) : '—'}</td><td><strong>{money(month.expectedTotal)}</strong></td><td className={month.reserveBalance < 0 ? 'table-warn' : 'table-good'}>{money(month.reserveBalance)}</td></tr>)}</tbody></table></div>;
}
...
function LegacyFAQView() {
  return <section className="page-section faq-page">
    <div className="page-heading"><div><p className="eyebrow">СПРАВКА / МЕТОДИКА</p><h1>Как работает «Автоосмотр»</h1><p className="muted">Краткое описание расчётов, порядка осмотра и правил интерпретации результата.</p></div></div>
    <div className="faq-lead"><strong>Принцип приложения:</strong> факты специалиста → детерминированные формулы → бюджетная зона → прогноз рисков → рейтинг. Приложение не принимает решение вместо владельца и не заменяет независимую техническую экспертизу.</div>
    <div className="faq-grid">
      <article className="content-card faq-card"><p className="eyebrow">01 / ОСМОТР</p><h2>Как проходить осмотр</h2><ol><li>Создайте осмотр только для одной из трёх поддерживаемых моделей.</li><li>Заполните цену, VIN, госномер и данные документов по мере их получения.</li><li>Последовательно проходите этапы: кузов, салон, двигатель, АКПП, ходовая, электрика и остальные блоки.</li><li>На каждом элементе выберите состояние и нажмите «Сохранить». Факт сразу попадает в журнал и расчёт.</li><li>Если точность факта сомнительна, оставьте статус «под вопросом»: он всё равно учитывается в смете.</li></ol></article>
      <article className="content-card faq-card"><p className="eyebrow">02 / БЮДЖЕТ</p><h2>Как считается доведение</h2><p>Цена расчёта — фактическая цена сделки или цена объявления минус ожидаемый торг.</p><div className="formula">Фонд доведения = общий фонд − цена расчёта<br />Безопасная стоимость = стоимость специалиста × K<br />Остаток = фонд доведения − работы «Сразу»</div><p>Текущая бюджетная зона использует только работы, которые нужно выполнить сразу после покупки. Отложенные работы не исчезают: они отображаются отдельно и переносятся в пятилетний прогноз.</p></article>
      <article className="content-card faq-card"><p className="eyebrow">03 / ПРИОРИТЕТЫ</p><h2>Что означает срочность</h2><table className="faq-table"><thead><tr><th>Приоритет</th><th>Текущий бюджет</th><th>Прогноз</th></tr></thead><tbody><tr><td>Сразу</td><td>Да</td><td>Не добавляется повторно</td></tr><tr><td>В ближайшее время</td><td>Нет</td><td>3-й месяц</td></tr><tr><td>Планово</td><td>Нет</td><td>9-й месяц</td></tr><tr><td>Желательно</td><td>Нет</td><td>18-й месяц</td></tr></tbody></table></article>
      <article className="content-card faq-card"><p className="eyebrow">04 / ЦВЕТОВАЯ ЗОНА</p><h2>Как читать запас</h2><table className="faq-table"><thead><tr><th>Зона</th><th>Условие</th><th>Смысл</th></tr></thead><tbody><tr><td className="faq-green">Зелёная</td><td>Запас ≥ 20%</td><td>Есть рабочий резерв</td></tr><tr><td className="faq-yellow">Жёлтая</td><td>10–20%</td><td>Нужна осторожность</td></tr><tr><td className="faq-red">Красная</td><td>&lt; 10% или отрицательный остаток</td><td>Дальнейший осмотр может быть нецелесообразен</td></tr></tbody></table><p>Критический кузовной риск, неизвестная стоимость работы и превышение фильтра цены дают отдельный блокирующий сигнал независимо от цвета.</p></article>
      <article className="content-card faq-card"><p className="eyebrow">05 / ПРОГНОЗ</p><h2>Как работают потенциальные события</h2><p>Вероятностное событие имеет вероятность за 5 лет, стоимость, коэффициент K, максимальную стоимость и временное окно. Ожидаемая стоимость считается так:</p><div className="formula">Ожидаемо = вероятность × стоимость × K</div><p>Событие с известным сроком считается обязательным в указанном месяце. Все события можно отключать, редактировать или удалять прямо в прогнозе. Вероятности являются сценарными параметрами, а не официальной статистикой.</p></article>
      <article className="content-card faq-card"><p className="eyebrow">05A / P И K</p><h2>Почему 20% и K 1,2 — не одно и то же</h2><p><strong>Вероятность P</strong> отвечает на вопрос «как часто событие может произойти за 5 лет». <strong>Коэффициент K</strong> отвечает на вопрос «насколько увеличить названную стоимость для безопасной оценки».</p><div className="formula">P = 20% · стоимость = 65 000 ₽ · K = 1,2<br />Ожидаемо = 0,20 × 65 000 × 1,2 = 15 600 ₽</div><p>K не превращается в 120% вероятности. Для события с известным сроком P автоматически равна 100%, а месяц задаётся явно.</p></article>
      <article className="content-card faq-card"><p className="eyebrow">05B / ПО МЕСЯЦАМ</p><h2>Как читать помесячный прогноз</h2><p>«Регулярные расходы» — топливо, налог, ОСАГО и плановые бюджеты, равномерно разложенные по месяцам. «В резерв» — ежемесячная доля ожидаемых будущих ремонтов. «Отложенные работы» и «события по сроку» показывают разовые траты в конкретном месяце.</p><p>«Ожидаемо всего» показывает денежную потребность месяца без маскировки пиков. «Баланс резерва» — условное накопление: пополнения минус ожидаемые разовые траты. Первичное доведение отображается отдельно до месяца 1.</p></article>
      <article className="content-card faq-card"><p className="eyebrow">06 / РЕЙТИНГ</p><h2>Из чего складываются баллы</h2><p>Рейтинг нормализуется до 100 баллов и учитывает соответствие бюджету, стоимость владения, годовые риски, частоту крупных ремонтов, максимальный ремонт, риски двигателя и АКПП, предсказуемость и обслуживание.</p><p>История и комплектность автомобиля влияют с небольшим весом: ДТП снижает этот компонент сильнее, дубликат ПТС — умеренно, отсутствие второго ключа — слегка. Это не автоматический отказ, а дополнительный фактор для сравнения.</p></article>
      <article className="content-card faq-card"><p className="eyebrow">07 / ДАННЫЕ</p><h2>Что сохраняется</h2><p>Осмотры и конфигурация хранятся локально в IndexedDB браузера. Данные можно экспортировать в JSON и импортировать на другом устройстве. Конфигурация фиксируется внутри каждого осмотра, поэтому старые результаты не меняются незаметно после редактирования глобальных настроек.</p></article>
      <article className="content-card faq-card"><p className="eyebrow">08 / КОНСТРУКТОР</p><h2>Можно ли изменить этапы осмотра</h2><p>Да. В осмотре нажмите «Настроить блоки». Можно переименовать этапы, добавить или удалить подблоки, добавить/переименовать/удалить отдельные элементы. Сохранённые факты остаются в журнале, даже если соответствующий элемент позже убрать из формы.</p></article>
      <article className="content-card faq-card"><p className="eyebrow">ЧАСТЫЕ ВОПРОСЫ</p><details open><summary>Как влияет факт «под вопросом»?</summary><p>Работа с указанной стоимостью учитывается в смете так же, как потенциальная трата. Состояние без стоимости не создаёт выдуманный ремонт и не снижает рейтинг; оно увеличивает счётчик неопределённых данных и показывает, что вывод требует подтверждения.</p></details><details><summary>Почему прогноз не повышает риск автоматически из-за неизвестности?</summary><p>Потому что это создало бы неподтверждённую статистику. Чтобы добавить финансовый риск, внесите конкретную работу, стоимость или отдельное потенциальное событие.</p></details><details><summary>Можно ли удалить стандартное событие?</summary><p>Да. На странице прогноза нажмите «Удалить» у нужной строки. Для этого осмотра событие исчезнет из модели; стандартная конфигурация не удаляется, а новые пользовательские работы добавляются там же.</p></details><details><summary>Что делать, если нужного элемента нет?</summary><p>Сохраните его через «Свободный факт» в общем журнале либо добавьте элемент через «Настроить блоки».</p></details><details><summary>Гарантирует ли зелёная зона, что автомобиль безопасен?</summary><p>Нет. Это расчётный запас по введённым данным. Структурная коррозия, плохая диагностика или пропущенные дефекты требуют решения специалиста.</p></details></article>
    </div>
  </section>;
}
...
function LegacyCompareView({ inspections, onOpen }: { inspections: Inspection[]; onOpen: (id: string) => void }) {
  const [sort, setSort] = useState('score');
  const rows = useMemo(() => inspections.map((inspection) => ({ inspection, result: calculateInspection(inspection) })).sort((left, right) => {
    if (sort === 'price') return left.result.calculationPrice - right.result.calculationPrice;
    if (sort === 'restore') return left.result.safeRestoreCost - right.result.safeRestoreCost;
  ...
  return <section className="page-section"><div className="page-heading"><div><p className="eyebrow">ИСТОРИЯ / СРАВНЕНИЕ</p><h1>Сравнить автомобили</h1><p className="muted">Только осмотры из разрешённого пула моделей.</p></div><label className="sort-control">Сортировать <select value={sort} onChange={(event) => setSort(event.target.value)}><option value="score">по рейтингу</option><option value="price">по цене</option><option value="restore">по доведению</option><option value="remaining">по остатку</option><option value="ownership">по владению</option></select></label></div>{rows.length === 0 ? <div className="empty-state"><div className="empty-icon">▦</div><h2>Сравнивать пока нечего</h2><p>Создайте хотя бы один осмотр, и он появится в этой таблице.</p></div> : <div className="content-card table-card"><div className="table-scroll"><table><thead><tr><th>Автомобиль</th><th>Цена</th><th>Доведение</th><th>Остаток</th><th>Опер. ₽/мес</th><th>Полные ₽/мес</th><th>Рейтинг</th><th>Зона</th></tr></thead><tbody>{rows.map(({ inspection, result }) => <tr key={inspection.id} className="clickable-row" onClick={() => onOpen(inspection.id)}><td><strong>{modelLabel(inspection.vehicle.modelId)}</strong><small>{inspection.vehicle.year} · {inspection.vehicle.mileage.toLocaleString('ru-RU')} км</small></td><td>{money(result.calculationPrice)}</td><td>{money(result.safeRestoreCost)}</td><td className={result.remainingBudget < 0 ? 'table-warn' : ''}>{money(result.remainingBudget)}</td><td>{money(result.forecast.averageMonthlyCost)}</td><td>{money(result.forecast.fullAverageMonthlyCost)}</td><td><strong>{result.rating.score ?? '—'}</strong></td><td><span className={`table-zone ${result.zone.toLowerCase()}`}>{zoneText(result.zone)}</span></td></tr>)}</tbody></table></div></div>}</section>;
}
...
function LegacySettingsViewV1({ config, active, onUpdate, onApplyActive }: { config: AppConfig; active: Inspection | null; onUpdate: (updater: (config: AppConfig) => AppConfig) => void; onApplyActive: () => void }) {
  const setScenarioNumber = (key: 'annualKm' | 'fuelPrice' | 'annualLimit', value: number) => onUpdate((current) => { current.scenario[key] = value; return current; });
  const setArrayValue = (key: 'insuranceByYear' | 'serviceByYear' | 'fluidsByYear' | 'consumablesByYear' | 'tiresByYear' | 'washingByYear' | 'finesByYear', index: number, value: number) => onUpdate((current) => { current.scenario[key][index] = value; return current; });
  const updateModel = (modelId: ModelId, field: 'consumptionLPer100Km' | 'taxAnnual', value: number) => onUpdate((current) => { const model = current.models.find((item) => item.id === modelId); if (model) model[field] = value; return current; });
  return <section className="page-section settings-page"><div className="page-heading"><div><p className="eyebrow">КОНФИГУРАЦИЯ</p><h1>Настройки модели</h1><p className="muted">Общие правила отдельно, а параметры каждого автомобиля — внутри своей группы.</p></div>{active && <button className="primary-button" onClick={onApplyActive}>Применить к текущему осмотру</button>}</div><div className="info-strip"><span className="info-icon">i</span><span>Вероятности и стоимости являются редактируемыми сценарными параметрами. Старые осмотры не меняются, пока вы явно не примените к ним текущую конфигурацию.</span></div><div className="settings-grid"><div className="content-card"><div className="section-heading compact-heading"><div><p className="eyebrow">ОБЩИЕ ПРАВИЛА</p><h2>Бюджет</h2></div></div><div className="form-grid three"><Field label="Общий фонд, ₽"><input type="number" value={config.fund} onChange={(event) => onUpdate((current) => { current.fund = numberValue(event.target.value); return current; })} /></Field><Field label="Макс. объявления, ₽"><input type="number" value={config.maxAskingPrice} onChange={(event) => onUpdate((current) => { current.maxAskingPrice = numberValue(event.target.value); return current; })} /></Field><Field label="Целевая цена, ₽"><input type="number" value={config.targetPurchasePrice} onChange={(event) => onUpdate((current) => { current.targetPurchasePrice = numberValue(event.target.value); return current; })} /></Field></div><div className="form-grid two"><Field label="Зелёная зона, %"><input type="number" value={config.greenReserveRatio * 100} onChange={(event) => onUpdate((current) => { current.greenReserveRatio = numberValue(event.target.value) / 100; return current; })} /></Field><Field label="Жёлтая зона, %"><input type="number" value={config.yellowReserveRatio * 100} onChange={(event) => onUpdate((current) => { current.yellowReserveRatio = numberValue(event.target.value) / 100; return current; })} /></Field></div></div><div className="content-card"><div className="section-heading compact-heading"><div><p className="eyebrow">ОБЩИЕ ПРАВИЛА</p><h2>Сценарий эксплуатации</h2></div></div><div className="form-grid three"><Field label="Лет"><input type="number" min="1" max="20" value={config.scenario.years} onChange={(event) => onUpdate((current) => { current.scenario.years = numberValue(event.target.value); return current; })} /></Field><Field label="Пробег в год, км"><input type="number" value={config.scenario.annualKm} onChange={(event) => setScenarioNumber('annualKm', numberValue(event.target.value))} /></Field><Field label="АИ-95, ₽/л"><input type="number" step="0.1" value={config.scenario.fuelPrice} onChange={(event) => setScenarioNumber('fuelPrice', numberValue(event.target.value))} /></Field></div><div className="form-grid four"><Field label="Лимит в год, ₽"><input type="number" value={config.scenario.annualLimit} onChange={(event) => setScenarioNumber('annualLimit', numberValue(event.target.value))} /></Field><Field label="Крупный ремонт, ₽"><input type="number" value={config.majorRepairThreshold} onChange={(event) => onUpdate((current) => { current.majorRepairThreshold = numberValue(event.target.value); return current; })} /></Field><Field label="Крупных в год"><input type="number" value={config.majorRepairsPerYearLimit} onChange={(event) => onUpdate((current) => { current.majorRepairsPerYearLimit = numberValue(event.target.value); return current; })} /></Field><Field label="Интервал, мес."><input type="number" value={config.minMonthsBetweenMajorRepairs} onChange={(event) => onUpdate((current) => { current.minMonthsBetweenMajorRepairs = numberValue(event.target.value); return current; })} /></Field></div><div className="year-inputs"><div className="year-header"><span>Расход</span>{Array.from({ length: config.scenario.years }, (_, index) => <span key={index}>Год {index + 1}</span>)}</div><YearInput label="ОСАГО" values={config.scenario.insuranceByYear} onChange={(index, value) => setArrayValue('insuranceByYear', index, value)} /><YearInput label="Плановое ТО" values={config.scenario.serviceByYear} onChange={(index, value) => setArrayValue('serviceByYear', index, value)} /><YearInput label="Жидкости" values={config.scenario.fluidsByYear} onChange={(index, value) => setArrayValue('fluidsByYear', index, value)} /><YearInput label="Расходники" values={config.scenario.consumablesByYear} onChange={(index, value) => setArrayValue('consumablesByYear', index, value)} /><YearInput label="Резина" values={config.scenario.tiresByYear} onChange={(index, value) => setArrayValue('tiresByYear', index, value)} /><YearInput label="Мойка" values={config.scenario.washingByYear} onChange={(index, value) => setArrayValue('washingByYear', index, value)} /><YearInput label="Штрафы" values={config.scenario.finesByYear} onChange={(index, value) => setArrayValue('finesByYear', index, value)} /></div></div></div><div className="content-card full-width"><div className="section-heading compact-heading"><div><p className="eyebrow">АВТОМОБИЛИ</p><h2>Параметры по моделям</h2></div><span className="muted">Расход, налог и события собраны внутри карточки каждой модели.</span></div><div className="model-settings-groups">{config.models.map((model) => <article className="model-settings-card" key={model.id}><div className="model-settings-title"><div><span className="step-chip">{model.generation}</span><h3>{modelLabel(model.id)}</h3><p>{model.engine} · {model.transmission}</p></div><div className="model-settings-inputs"><Field label="Расход, л/100 км"><input type="number" step="0.1" value={model.consumptionLPer100Km} onChange={(event) => updateModel(model.id, 'consumptionLPer100Km', numberValue(event.target.value))} /></Field><Field label="Налог, ₽/год"><input type="number" value={model.taxAnnual} onChange={(event) => updateModel(model.id, 'taxAnnual', numberValue(event.target.value))} /></Field></div></div><div className="engine-variant-list"><strong>Варианты двигателя и ГРМ</strong>{model.engineVariants.map((variant) => <span key={variant.id} className="engine-variant-pill">{variant.label}</span>)}</div><div className="model-event-group"><div className="section-heading compact-heading"><div><p className="eyebrow">СОБЫТИЯ ЭТОЙ МОДЕЛИ</p><h3>Потенциальные ремонты</h3></div><span className="muted">P — вероятность за 5 лет, K — множитель стоимости.</span></div><div className="event-settings">{config.repairEvents.filter((event) => event.modelIds.length === 1 && event.modelIds.includes(model.id)).map((event) => <SettingsEventRow key={event.id} event={event} onUpdate={onUpdate} />)}</div></div></article>)}</div><article className="model-settings-card common-events-card"><div className="model-settings-title"><div><span className="step-chip">ОБЩИЕ</span><h3>События для всех трёх моделей</h3><p>Одинаковые расходники и типовые работы.</p></div></div><div className="event-settings">{config.repairEvents.filter((event) => event.modelIds.length > 1).map((event) => <SettingsEventRow key={event.id} event={event} onUpdate={onUpdate} />)}</div></article></div><div className="content-card full-width"><div className="section-heading compact-heading"><div><p className="eyebrow">ОБЩИЕ ПРАВИЛА</p><h2>Рейтинг</h2></div><span className="muted">Сумма весов автоматически нормализуется до 100 баллов.</span></div><div className="coefficient-grid">{(Object.keys(config.ratingWeights) as Array<keyof AppConfig['ratingWeights']>).map((key) => <label key={key}><span>{RATING_WEIGHT_LABELS[key]}</span><input type="number" min="0" value={config.ratingWeights[key]} onChange={(event) => onUpdate((current) => { current.ratingWeights[key] = numberValue(event.target.value); return current; })} /></label>)}</div></div><div className="content-card full-width"><div className="section-heading compact-heading"><div><p className="eyebrow">ОБЩИЕ ПРАВИЛА</p><h2>Коэффициенты неопределённости</h2></div><span className="muted">K увеличивает безопасную оценку, но не является вероятностью.</span></div><div className="coefficient-grid">{config.coefficients.map((rule) => <label key={rule.id}><span>{rule.label}</span><input type="number" step="0.01" min="1" value={rule.coefficient} onChange={(event) => onUpdate((current) => { const target = current.coefficients.find((item) => item.id === rule.id)!; target.coefficient = numberValue(event.target.value); return current; })} /></label>)}</div></div></section>;
}
...
function LegacySettingsView({ config, active, onUpdate, onApplyActive }: { config: AppConfig; active: Inspection | null; onUpdate: (updater: (config: AppConfig) => AppConfig) => void; onApplyActive: () => void }) {
  const setScenarioNumber = (key: 'annualKm' | 'fuelPrice' | 'annualLimit', value: number) => onUpdate((current) => { current.scenario[key] = value; return current; });
  const setArrayValue = (key: 'insuranceByYear' | 'serviceByYear' | 'fluidsByYear' | 'consumablesByYear' | 'tiresByYear' | 'washingByYear' | 'finesByYear', index: number, value: number) => onUpdate((current) => { current.scenario[key][index] = value; return current; });
  return <section className="page-section settings-page"><div className="page-heading"><div><p className="eyebrow">КОНФИГУРАЦИЯ</p><h1>Настройки модели</h1><p className="muted">Все значения видимы и входят в версию расчёта.</p></div>{active && <button className="primary-button" onClick={onApplyActive}>Применить к текущему осмотру</button>}</div><div className="info-strip"><span className="info-icon">i</span><span>Значения расходов и вероятностей — редактируемые модельные параметры. Они не претендуют на статистическую точность.</span></div><div className="settings-grid"><div className="content-card"><div className="section-heading compact-heading"><div><p className="eyebrow">БЮДЖЕТ</p><h2>Финансовые ограничения</h2></div></div><div className="form-grid three"><Field label="Общий фонд, ₽"><input type="number" value={config.fund} onChange={(event) => onUpdate((current) => { current.fund = numberValue(event.target.value); return current; })} /></Field><Field label="Макс. объявления, ₽"><input type="number" value={config.maxAskingPrice} onChange={(event) => onUpdate((current) => { current.maxAskingPrice = numberValue(event.target.value); return current; })} /></Field><Field label="Целевая цена, ₽"><input type="number" value={config.targetPurchasePrice} onChange={(event) => onUpdate((current) => { current.targetPurchasePrice = numberValue(event.target.value); return current; })} /></Field></div><div className="form-grid two"><Field label="Зелёная зона, %"><input type="number" step="1" value={config.greenReserveRatio * 100} onChange={(event) => onUpdate((current) => { current.greenReserveRatio = numberValue(event.target.value) / 100; return current; })} /></Field><Field label="Жёлтая зона, %"><input type="number" step="1" value={config.yellowReserveRatio * 100} onChange={(event) => onUpdate((current) => { current.yellowReserveRatio = numberValue(event.target.value) / 100; return current; })} /></Field></div></div><div className="content-card"><div className="section-heading compact-heading"><div><p className="eyebrow">ЭКСПЛУАТАЦИЯ</p><h2>Сценарий на 5 лет</h2></div></div><div className="form-grid three"><Field label="Лет"><input type="number" min="1" max="20" value={config.scenario.years} onChange={(event) => onUpdate((current) => { current.scenario.years = numberValue(event.target.value); return current; })} /></Field><Field label="Пробег в год, км"><input type="number" value={config.scenario.annualKm} onChange={(event) => setScenarioNumber('annualKm', numberValue(event.target.value))} /></Field><Field label="АИ-95, ₽/л"><input type="number" step="0.1" value={config.scenario.fuelPrice} onChange={(event) => setScenarioNumber('fuelPrice', numberValue(event.target.value))} /></Field></div><div className="form-grid two"><Field label="Лимит расходов в год, ₽"><input type="number" value={config.scenario.annualLimit} onChange={(event) => setScenarioNumber('annualLimit', numberValue(event.target.value))} /></Field><Field label="Сценариев симуляции"><input type="number" min="1000" value={config.simulationScenarios} onChange={(event) => onUpdate((current) => { current.simulationScenarios = numberValue(event.target.value); return current; })} /></Field></div><div className="form-grid four"><Field label="Крупный ремонт, ₽"><input type="number" value={config.majorRepairThreshold} onChange={(event) => onUpdate((current) => { current.majorRepairThreshold = numberValue(event.target.value); return current; })} /></Field><Field label="Критический, ₽"><input type="number" value={config.criticalRepairThreshold} onChange={(event) => onUpdate((current) => { current.criticalRepairThreshold = numberValue(event.target.value); return current; })} /></Field><Field label="Крупных в год"><input type="number" value={config.majorRepairsPerYearLimit} onChange={(event) => onUpdate((current) => { current.majorRepairsPerYearLimit = numberValue(event.target.value); return current; })} /></Field><Field label="Мин. интервал, мес."><input type="number" value={config.minMonthsBetweenMajorRepairs} onChange={(event) => onUpdate((current) => { current.minMonthsBetweenMajorRepairs = numberValue(event.target.value); return current; })} /></Field></div><div className="year-inputs"><div className="year-header"><span>Расход</span>{Array.from({ length: config.scenario.years }, (_, index) => <span key={index}>Год {index + 1}</span>)}</div><YearInput label="ОСАГО" values={config.scenario.insuranceByYear} onChange={(index, value) => setArrayValue('insuranceByYear', index, value)} /><YearInput label="Плановое ТО" values={config.scenario.serviceByYear} onChange={(index, value) => setArrayValue('serviceByYear', index, value)} /><YearInput label="Жидкости" values={config.scenario.fluidsByYear} onChange={(index, value) => setArrayValue('fluidsByYear', index, value)} /><YearInput label="Расходники" values={config.scenario.consumablesByYear} onChange={(index, value) => setArrayValue('consumablesByYear', index, value)} /><YearInput label="Резина" values={config.scenario.tiresByYear} onChange={(index, value) => setArrayValue('tiresByYear', index, value)} /><YearInput label="Мойка" values={config.scenario.washingByYear} onChange={(index, value) => setArrayValue('washingByYear', index, value)} /><YearInput label="Штрафы" values={config.scenario.finesByYear} onChange={(index, value) => setArrayValue('finesByYear', index, value)} /></div></div><div className="content-card"><div className="section-heading compact-heading"><div><p className="eyebrow">МОДЕЛИ</p><h2>Расход и налог</h2></div></div>{config.models.map((model) => <div className="model-setting" key={model.id}><div><strong>{modelLabel(model.id)}</strong><small>{model.generation} · {model.engine} · {model.transmission}</small></div><label>л/100 км<input type="number" step="0.1" value={model.consumptionLPer100Km} onChange={(event) => onUpdate((current) => { const target = current.models.find((item) => item.id === model.id)!; target.consumptionLPer100Km = numberValue(event.target.value); return current; })} /></label><label>налог/год<input type="number" value={model.taxAnnual} onChange={(event) => onUpdate((current) => { const target = current.models.find((item) => item.id === model.id)!; target.taxAnnual = numberValue(event.target.value); return current; })} /></label></div>)}</div></div><div className="content-card full-width"><div className="section-heading compact-heading"><div><p className="eyebrow">РЕЙТИНГ</p><h2>Весовые коэффициенты</h2></div><span className="muted">Сумма автоматически нормализуется до 100 баллов.</span></div><div className="coefficient-grid">{(Object.keys(config.ratingWeights) as Array<keyof AppConfig['ratingWeights']>).map((key) => <label key={key}><span>{RATING_WEIGHT_LABELS[key]}</span><input type="number" min="0" value={config.ratingWeights[key]} onChange={(event) => onUpdate((current) => { current.ratingWeights[key] = numberValue(event.target.value); return current; })} /></label>)}</div></div><div className="content-card full-width"><div className="section-heading compact-heading"><div><p className="eyebrow">КОЭФФИЦИЕНТЫ</p><h2>Неопределённость стоимости</h2></div><span className="muted">Изменение создаёт новую версию конфигурации.</span></div><div className="coefficient-grid">{config.coefficients.map((rule) => <label key={rule.id}><span>{rule.label}</span><input type="number" step="0.01" min="1" value={rule.coefficient} onChange={(event) => onUpdate((current) => { const target = current.coefficients.find((item) => item.id === rule.id)!; target.coefficient = numberValue(event.target.value); return current; })} /></label>)}</div></div><div className="content-card full-width"><div className="section-heading compact-heading"><div><p className="eyebrow">СОБЫТИЯ РЕМОНТА</p><h2>Модельные вероятности</h2></div><span className="muted">Вероятность вводится за весь пятилетний период.</span></div><div className="event-settings">{config.repairEvents.map((event) => <div className="event-setting" key={event.id}><div className="event-setting-name"><strong>{event.name}</strong><small>{modelLabel(event.modelIds[0])} · {categoryName(event.category)}</small></div><label>P, %<input type="number" min="0" max="100" value={event.probability5y * 100} onChange={(input) => onUpdate((current) => { const target = current.repairEvents.find((item) => item.id === event.id)!; target.probability5y = numberValue(input.target.value) / 100; return current; })} /></label><label>Стоимость<input type="number" value={event.repairCost} onChange={(input) => onUpdate((current) => { const target = current.repairEvents.find((item) => item.id === event.id)!; target.repairCost = numberValue(input.target.value); return current; })} /></label><label>Коэфф. K<input type="number" step="0.01" value={event.coefficient} onChange={(input) => onUpdate((current) => { const target = current.repairEvents.find((item) => item.id === event.id)!; target.coefficient = numberValue(input.target.value); return current; })} /></label><label>Макс. стоимость<input type="number" value={event.maxCost} onChange={(input) => onUpdate((current) => { const target = current.repairEvents.find((item) => item.id === event.id)!; target.maxCost = numberValue(input.target.value); return current; })} /></label><label>Месяцы<input type="text" value={`${event.monthStart}-${event.monthEnd}`} onChange={(input) => { const [start, end] = input.target.value.split('-').map(numberValue); onUpdate((current) => { const target = current.repairEvents.find((item) => item.id === event.id)!; target.monthStart = start || 1; target.monthEnd = end || start || 1; return current; }); }} /></label></div>)}</div></div></section>;
}
